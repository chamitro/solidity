...
string YulUtilFunctions::allocateAndInitializeMemoryStructFunctionZero(StructType const& _type)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		...
		else
		{
			...
			if (auto const* arrayType = dynamic_cast<ArrayType const*>(&_type))
			{
				...
				templ("zeroValue", allocateAndInitializeMemoryStructFunctionZero(*structType) + "()");
			else
				solUnimplementedAssert(false, "");
		}
